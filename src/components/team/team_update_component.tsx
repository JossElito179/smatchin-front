...
const TeamUpdateComponent = ({ id }: string | any) => {
    ...
    const [teamToUpdate, setTeamToUpdate] = useState<any>(null);
    const [userData, setUserData] = useState<any>(null);

    const navigate = useNavigate();


    async function fetchTeamToUpdate() {
        try {
            setLoading(true);
            ...
            setExistingTeamImg(data.team_img);
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
    ...
    const handleSubmit = () => {
        const addOwnerData = async () => {
            try {
                ...
                const response = await axios.patch(endpoint + 'teams/' + id, formData, {
                    headers: {
