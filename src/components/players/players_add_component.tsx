...
const PlayerAddComponent = ({ id_teams, name }: { id_teams: string | undefined; name: string | undefined}) => {
    ...
    const handleSubmit = () => {
        const addOwnerData = async () => {
            try {
                ...
                formData.append('id_teams', id_teams+'');
                ...
                const response = await axios.post(endpoint+'players', formData, {
                    headers: {
    ...
    return (
        <Box sx={{ py: 4, px: 2 }}>
                ...
                <IconButton
                    onClick={() => navigate('/players/' + id_teams+ '/'+ name)}
                    color="primary"
                    aria-label="retour"
                    sx={{
                        marginBottom: 2,
